...
export function PipelineDonut({ data, loading }: PipelineDonutProps) {
  return (
    <section className="flex h-full flex-col rounded-2xl bg-card ring-1 ring-foreground/[0.06] shadow-sm">
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Pipeline value by stage">
        {/* background ring */}
        <circle cx={cx} cy={cy} r={r} fill="none" className="stroke-border" strokeWidth={ringWidth} />
        {segments.map((seg) => (
          <path
            key={seg.id}
            d={seg.path}
